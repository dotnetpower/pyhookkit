...
class TeamsGraphMembershipProvisioner:
    """Ensure the Teams connector identity is a direct Team member."""
    ...
    def ensure_member(
        ...
        raise TeamsGraphMembershipError(
            f"Microsoft Graph Team member creation failed with HTTP "
    ...
    def is_member(self, team_id: UUID, user_id: UUID) -> bool:
        """Return whether the user is a direct Team member."""
        next_url: str | None = f"{_GRAPH_ORIGIN}/v1.0/teams/{team_id}/members"
        while next_url is not None:
            _validate_graph_url(next_url)
            response = self._send("GET", next_url)
            if response.status_code != 200:
                raise TeamsGraphMembershipError(
                    f"Microsoft Graph Team member lookup failed with HTTP "
                    f"{response.status_code}"
                )
            payload = _response_object(response, "Team member lookup")
            raw_members = payload.get("value")
            if not isinstance(raw_members, list):
                raise TeamsGraphMembershipError(
                    "Microsoft Graph Team member lookup response is malformed"
                )
            ...
            if raw_next_link is not None and not isinstance(raw_next_link, str):
                raise TeamsGraphMembershipError(
                    "Microsoft Graph Team member next link is malformed"
                )
...
def _validate_graph_url(url: str) -> None:
    ...
    if (
        ...
        raise TeamsGraphMembershipError(
            "Microsoft Graph Team member next link has an unexpected origin"
        )
