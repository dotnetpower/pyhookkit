...
Optional live delivery:
  --smoke-test
  Requires TEAMS_WORKFLOW_URL and TEAMS_WORKFLOW_CHANNEL_LINK in the environment.
EOF
}
...
if [[ -n "$channel_report" ]]; then
  if [[ -z "$inventory_team_id" ]]; then
    echo "error: --inventory-team-id is required with --channel-report" >&2
    exit 2
  fi
